...
"""
IMU 균형 이상 감지 — Discriminative AE 추론 스크립트
...
def build_windows_from_series(df: pd.DataFrame, win_sec: int, stride_sec: int,
    ...
    items = [WindowData(feats=ws[i], label=0, group=group) for i in range(ws.shape[0])]

    # 윈도우 시작/끝 timestamp 계산
...
def run_inference(args: InferArgs):
    ...
    df = read_all_series_from_zip(zip_path, args.target_hz,
    ...
    print(f"[INFO] Data length (samples): {len(df)}, time range: {df.index.min()} ~ {df.index.max()}")

    print("[INFO] Building windows...")
    items, win_meta = build_windows_from_series(
        df, args.window_sec, args.stride_sec,
        target_hz=args.target_hz, group=zip_path.stem,
        trim_sec=args.trim_sec, excluded_axes=args.exclude_axes
    )
    if not items:
        raise RuntimeError("No windows created from series. Check window/stride/trim settings.")
    ...
    # 간단히 현재 데이터에서 채널 통계 추정 (학습 통계 별도 보관했다면 여기서 로드하도록 수정 가능)
    ...
    print(f"[INFO] Abnormal windows (label=1): {out_df['label'].sum()} / {len(out_df)} "
...
if __name__ == "__main__":
    ...
    p.add_argument("--zip_path", type=str, required=True,
                   help="추론할 IMU ZIP 파일 경로 (예: ./x_trimmed.zip)")
    p.add_argument("--ckpt_path", type=str, required=True,
                   help="학습된 ckpt(.pt) 경로 (예: ./fold3_ckpt.pt)")
    p.add_argument("--output_csv", type=str, default="./inference_result.csv",
    ...
    p.add_argument("--threshold", type=float, default=0.355,
